...
class AttentionForVllmMHA(nn.Module, AttentionLayerBase):
    ...
    def rope_cache(
        ...
        # kernel block size 16 (asm_pa_configs.hpp). When the KV cache uses a
        # different block size (e.g. the Eagle3 draft sharing the model's block
        # 128 so it can join vLLM's uniform-type per-layer-tensor KV cache group
        # instead of collapsing to a singleton group), asm has no kernel and the
        # worker crashes ("cannot get heuristic kernel"). The Triton path
        # (insert + decode) is block-size agnostic, so route bf16 caches with a
        # non-16 block size through Triton.
        use_triton_attn = (
    ...
    def get_kv_cache_spec(self, vllm_config):
        ...
        # sliding window (draft config: sliding_window=null), so it must be a
        # FullAttentionSpec. MLAAttentionSpec (the sparse indexer cache)
        # subclasses FullAttentionSpec, so a spec set of {full main, MLA index,
        # full draft} is uniform-type and vLLM allocates a separate KV tensor
        # per layer (UniformTypeKVCacheSpecs path). A SlidingWindowSpec draft is
        # NOT a FullAttentionSpec subclass, which breaks uniformity and drops
        # M3 onto the hybrid/general allocator; with a single draft layer that
        # collapses group_size to 1 and packs every layer into one shared
        # KVCacheTensor, aliasing the dense (K-first) and sparse (KV-interleaved)
        # layouts and corrupting the KV cache.
