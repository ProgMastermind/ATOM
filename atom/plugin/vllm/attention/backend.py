...
class AiterMhaBackendForVllm:
    ...
    def get_supported_kernel_block_sizes():
        ...
        # for kernel block size 16, but the Triton paged-attention path
        # (paged_attention_triton / run_pa_decode_gluon, plus the Triton
        # reshape_and_cache insert) reads block_size from the cache shape at
        # runtime and handles any multiple of 16. AttentionForVllmMHA routes to
        # the Triton path whenever the bf16 KV cache block size is not 16 (see
        # layer_mha.use_triton_attn), so this backend genuinely supports any
        # MultipleOf(16). Declaring it as such lets vLLM pick the kv-manager
        # block size (e.g. 128) as the common kernel block size, so a layer using
        # this backend (the Eagle3 draft) can share the uniform-type KV cache
        # group with the block-128 sparse/dense layers instead of forcing a
        # singleton group (group_size=1 -> shared-tensor aliasing) or a
