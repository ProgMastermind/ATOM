...
class _V4RoPE(nn.Module):
    ...
    def inverse(self, positions: torch.Tensor, x: torch.Tensor, prefix: str = "") -> None:
        """In-place inverse RoPE via fused Triton kernel.

        ``x`` must be the rope-slice only (last dim == rotary_dim).
        """
        inverse_rope_inplace(x, self.cos_cache, self.sin_cache, positions, prefix=prefix)
