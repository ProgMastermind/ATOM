...
def _decode_index_score_kernel(
    ...
    for blk in tl.range(chunk_start_block, chunk_end_block):
        ...
        # Forced blocks are overwritten below, so skip their K-cache loads while
        # keeping the original single-loop control flow for Triton stability.
        score_from_cache = (blk >= init_blocks) & (blk < local_start)
        k = tl.load(
            ik_cache_ptr
            + page * stride_ik_blk
            + off_k[None, :] * stride_ik_pos
            + off_d[:, None] * stride_ik_d,
            mask=d_mask[:, None] & pos_mask[None, :] & score_from_cache,
            other=0.0,
        ).to(
            tl.float32
        )  # [D, N]
