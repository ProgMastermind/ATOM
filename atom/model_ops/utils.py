...
def reshape_mxfp4_scale_for_triton(
    scale: torch.Tensor,
    rows: int,
    block_size: int = MXFP4_QUANT_BLOCK_SIZE,
) -> torch.Tensor:
    """Return MXFP4 scales in Triton's grouped-row layout."""
    if rows >= block_size:
        scale = scale.view(torch.uint8)
        num_row_blocks = (scale.shape[0] + block_size - 1) // block_size
        padded_rows = num_row_blocks * block_size
        if scale.shape[0] != padded_rows:
            pad = torch.zeros(
                (padded_rows - scale.shape[0], *scale.shape[1:]),
                dtype=scale.dtype,
                device=scale.device,
            )
            scale = torch.cat((scale, pad), dim=0)
        return scale.view(num_row_blocks, -1)
    return scale[:rows, ...].view(torch.uint8)
