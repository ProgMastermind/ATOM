...
class ATOMKVByteCodec:
    ...
    def host_to_gpu_block_range(
        ...
        block_ids: list[int],
        stream: torch.cuda.Stream | None = None,
        src_block_count: int | None = None,
    ) -> None:
        """H2D a logical block subrange from a host buffer.

        ``host_buf`` is a complete LMCache object containing ``src_block_count``
        logical blocks in this codec's current layout. Copy the subrange starting
        at ``src_block_start`` into ``block_ids``. This is needed when the first
        LMCache chunk overlaps an HBM prefix-cache hit: the host object contains
        the full chunk, but only the private tail blocks should be restored.
        """
        src_block_start = operator.index(src_block_start)
        block_ids = self._normalize_block_ids(block_ids)
        if src_block_count is None:
            if int(host_buf.numel()) % self.bytes_per_block != 0:
                raise ValueError(
                    "ATOMKVByteCodec: host_buf size is not block-aligned"
                )
            src_block_count = int(host_buf.numel()) // self.bytes_per_block
        src_block_count = operator.index(src_block_count)
        if src_block_start < 0:
            raise ValueError("ATOMKVByteCodec: src_block_start must be non-negative")
        if src_block_count < 0:
            raise ValueError("ATOMKVByteCodec: src_block_count must be non-negative")
        if src_block_start + len(block_ids) > src_block_count:
            raise ValueError(
                "ATOMKVByteCodec: source block range is out of bounds "
                f"start={src_block_start} blocks={len(block_ids)} "
                f"source_blocks={src_block_count}"
            )
        self._validate_host_buf(host_buf, src_block_count)
        if not block_ids:
            return

        if self.layout == "block":
            offset = src_block_start * self.bytes_per_block
            self.host_to_gpu(host_buf[offset:], block_ids, stream)
            return
        ...
        with self._device_ctx():
            ...
            with stream_ctx:
                if self.layout == "segment_indexed":
                    ...
                    bases = self._segment_bases(src_block_count)
                    for seg, base, nb in zip(
                        self._segments, bases, self._seg_block_bytes
                    ):
                        mat = self._segment_bytes_matrix(seg)
                        tmp = self._tmp_bytes(seg, len(block_ids))
                        start = base + src_block_start * nb
                        tmp.copy_(
                            host_buf[start : start + len(block_ids) * nb].reshape_as(
                                tmp
                            ),
                            non_blocking=True,
                        )
                ...
                for seg, base, nb in zip(
                    ...
                    for logical_start, physical_start, run_len in runs:
                        dst = self._blocks_bytes_view(seg, physical_start, run_len)
                        src = base + (src_block_start + logical_start) * nb
                        dst.copy_(
                            host_buf[src : src + run_len * nb],
                            non_blocking=True,
                        )
