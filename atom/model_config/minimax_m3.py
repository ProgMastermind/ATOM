...
class MiniMaxM3TextConfig(PretrainedConfig):
    ...
    Defaults mirror the MiniMax-M3 checkpoint text_config. Some public
    checkpoints omit ``swiglu_beta``; MiniMax-M3's SwiGLU-OAI activation uses
    beta=1.0, matching ``gate * sigmoid(alpha * gate) * (up + 1)``.
    """
...
class MiniMaxM3Config(PretrainedConfig):
    """Top-level MiniMax-M3 VL config used to expose the text backbone."""
...
__all__ = ["MiniMaxM3Config", "MiniMaxM3TextConfig"]
# SPDX-License-Identifier: Apache-2.0
# SPDX-FileCopyrightText: Copyright contributors to the vLLM project
from typing import Any
